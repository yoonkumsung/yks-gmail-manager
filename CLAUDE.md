...
3. **완결된 요약 (본문 기반)**: 메일 본문에 충실한 설명이 있으면 요약만 읽어도 될 정도로 핵심사실+수치+배경+시사점을 담아 완결 요약(major tier). 분량은 본문 비례(충실한 본문 300~500자, 짧은 본문 50~200자). 단 본문에 없는 내용을 지어내지 않음.
4. **티저는 링크 위임 (brief tier)**: 본문이 제목·한두 문장 티저뿐인 매체는 본문에 있는 만큼만 한 줄로 요약하고 원문 링크로 위임(brief tier). 깊은 내용은 생성된 MD의 링크로 독자가 직접 확인. 크롤(원문 전문 가져오기)은 기본 비활성(옵션 `ENABLE_CRAWL`).
...
```
Gmail → HTML→텍스트 → [원문링크 크롤링: 옵션 ENABLE_CRAWL, 기본 off] → LLM 추출 → LLM 병합 → HTML/MD 리포트
```
