...
> **systemd user 타이머**가 매일 09:40 KST에 다이제스트를 직접 실행하도록 전환했다.
...
- 스케줄 윈도우는 코드가 KST 기준 **전날 09:41 ~ 당일 09:40**으로 계산(누락 없는 24h 타일링).
- 노트북이 9시 40분에 꺼져 있었으면 `Persistent=true`로 **켜진 직후 1회 보충 실행**.
