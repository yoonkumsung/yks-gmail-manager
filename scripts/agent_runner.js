...
[규칙]
1. 모든 뉴스를 개별 아이템으로 추출. 광고/푸터/구독안내만 제외. 같은 기사를 2개로 분리 추출하지 말 것.
...
[병합 규칙]
- [최우선] 같은 원문 link(URL)를 가진 아이템은 같은 출처(source_email)라도 반드시 하나로 병합한다. 특히 짧은 티저 요약과 그 기사의 상세 전문(=== 원문 기사 전문 ===에서 온 충실한 본문)이 같은 link면 동일 기사이므로 전문 쪽 내용을 기준으로 병합한다.
...
[분리 규칙]
- link가 다르면 절대 병합하지 않는다 (서로 다른 기사이므로 오병합 금지)
