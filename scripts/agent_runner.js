...
class AgentRunner {
  ...
  getTaskConfig(taskType) {
    const configs = {
      extract: {
        systemPrompt: `뉴스레터 본문(메일 텍스트)에서 모든 뉴스 아이템을 빠짐없이 추출하는 전문가. 입력은 메일 본문뿐이며, 외부 원문 크롤링은 없다.

[규칙]
1. 모든 뉴스를 개별 아이템으로 추출. 광고/푸터/구독안내만 제외. 같은 기사를 2개로 분리 추출하지 말 것. 특히 같은 기사가 본문 상세 소개와 "이번 주 소식" 같은 목록 양쪽에 나오면 하나의 아이템으로만 추출하되, 목록 쪽에만 있는 link를 그 아이템의 link 필드에 채운다.
2. 요약은 본문에 실제로 담긴 내용의 깊이에 비례:
   - 본문이 충실한 기사: 200~500자로 핵심사실+수치+배경을 충실히 요약.
   - 티저(제목+한두 줄만 있는 항목): 본문에 있는 만큼만 1~2줄로 요약(50~140자, 부풀리기 금지). 깊이는 link로 위임한다.
3. 영문은 자연스러운 한국어 번역. 고유명사 원어 병기.
4. link 필드에 원문 URL을 반드시 추출(특히 티저 항목은 클릭 이동이 목적이므로 link 필수). 마크다운 [텍스트](URL)에서는 URL만. 없으면 빈 문자열.

[절대 금지]
- 할루시네이션: 본문에 없는 수치·인물·사실을 절대 생성 금지(티저든 충실 기사든 동일하게 엄격).
- 금지 표현(충실 기사용 요약에서): "확인할 수 있다", "자세한 내용은" 등으로 내용을 회피하지 말 것. 단 티저 항목은 짧게 끝내고 link로 위임해도 됨.
- 정보 부족 시: 있는 내용만 쓰고 마침표로 끝냄. 모르면 안 씀.

[출력] 순수 JSON만. 설명/코드블록 없이 {로 시작 }로 끝남.`,
        temperature: 0.1,
        tailInstruction: '위 에이전트 지시사항과 SKILL 규칙에 따라 모든 뉴스 아이템을 빠짐없이 추출하고 JSON으로 출력하세요. 하나라도 누락하면 안 됩니다. 최우선 규칙: 입력 텍스트에 없는 내용을 절대 지어내지 마시오. 본문이 충실한 아이템은 200자 이상으로, 티저 항목은 본문에 있는 만큼만 짧게 요약하고 link를 반드시 채우시오.'
      },
      analyze: {
...
[병합 규칙]
- [최우선] 같은 원문 link(URL)를 가진 아이템은 같은 출처(source_email)라도 반드시 하나로 병합한다. 같은 link면 가장 충실한(summary가 가장 긴) 아이템을 기준으로 병합한다.
- 같은 사건/발표/수치 → 병합: 가장 내용이 충실한(summary가 긴) 아이템을 기준으로 삼고, 다른 아이템에만 있는 추가 정보를 기준 아이템의 summary에 합침
- [한쪽만 link 있음] 한 아이템은 link가 있고 다른 아이템은 link가 빈 문자열인데 같은 사건/기사이면(제목·요약이 같은 내용) 병합한다. 이는 '다른 link'가 아니다(같은 기사가 본문 상세와 목록에서 이중 추출된 경우). 병합 결과 link에는 있는 쪽 link를 채운다.
- 병합 시 모든 source를 콤마로 연결 (예: "A뉴스레터, B뉴스레터")
- 병합 시 keywords도 합집합으로 통합

[분리 규칙]
- 둘 다 link가 있고 서로 다르면 절대 병합하지 않는다 (서로 다른 기사이므로 오병합 금지). ※ 한쪽 link가 빈 문자열인 경우는 '다른 link'가 아니다 — 위 병합 규칙을 따른다.
