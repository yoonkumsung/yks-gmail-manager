...
function generateCombinedHtmlReportV2(allLabelsData, date, crossInsight, excludedMails = [], runStats = null) {
...
function generateStyles(labelColorCss) {
  return `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ============================================
       미니맵 (데스크톱 좌측, 컨테이너 여백 기준 균형)
       ============================================ */
    .minimap {
      position: fixed;
      /* 컨테이너(max-width 900px)의 좌측 바깥, 뷰포트 중앙에서 계산 */
      left: calc(50vw - 450px - 200px);
      top: 50%;
      transform: translateY(-50%);
      width: 180px;
      max-height: 70vh;
    ...
    /* 미니맵 표시: 컨테이너(900) + 미니맵(180) + 여백(40*2) = 1160px 이상 */
    @media (min-width: 1160px) {
      .minimap { display: block; }
    }
    /* 화면이 매우 넓을 때: 컨테이너 바깥 좌측 여백의 중앙에 배치 */
    @media (min-width: 1400px) {
      .minimap {
        left: calc((50vw - 450px) / 2 - 90px);
      }
    }
...
function generateFromMergedDir(mergedDir, outputPath, date) {
  const allLabelsData = [];
  ...
  const html = generateCombinedHtmlReportV2(filteredLabelsData, date, crossInsight, allExcluded, runStats);
  fs.writeFileSync(outputPath, html, 'utf8');
  console.log(`✓ v2 리포트 생성: ${outputPath}`);
  return outputPath;
}

module.exports = {
  generateCombinedHtmlReportV2,
  generateFromMergedDir,
  LABEL_COLORS,
  getLabelColor
};
