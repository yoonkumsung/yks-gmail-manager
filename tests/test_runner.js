...
async function runTests() {
  ...
  // 자동 로드 제외: test_runner 자체 + 라이브 전용 standalone 스크립트
  // (test_all_newsletters/quality_audit/recheck은 require 시 process.exit 호출하는 standalone 스크립트)
  const STANDALONE_SCRIPTS = new Set(['test_runner.js', 'test_all_newsletters.js', 'test_quality_audit.js', 'test_recheck.js']);
  const testFiles = fs.readdirSync(testDir)
    .filter(f => f.startsWith('test_') && f.endsWith('.js') && !STANDALONE_SCRIPTS.has(f))
